...
def test_tool_call_validation_accepts_dict_arguments(monkeypatch):
    ...
    # Two work calls consume the non-reserved budget and the third slot is used
    # for the bounded final synthesis attempt. The fake provider keeps returning
    # a tool call, so Hermes must stop with a visible explanation instead of
    # exceeding the hard model-call cap.
    assert result["completed"] is False
    assert result["run"]["budget"]["used"] == 3
    assert result["run"]["budget"]["remaining"] == 0
